<template>
  <div class="app-container">
    ...
    <el-card class="operate-container" shadow="never">
      ...
      <span>数据列表</span>
      <el-button class="btn-add" @click="handleDelete()" size="mini">
        删除
      </el-button>
      <el-button class="btn-add" @click="handleSubmit()" size="mini">
        提交申请
      </el-button>
      <el-button class="btn-add" @click="handleAddApplication()" size="mini">
        添加
      </el-button>
    </el-card>
      ...
      >
        ...
        <el-table-column label="申请单单号" width="100" align="center">
          <template slot-scope="scope">{{
            scope.row.applicationForm.formCode
          }}</template>
        </el-table-column>
        <el-table-column label="申请人" width="100" align="center">
          <template slot-scope="scope">{{
            scope.row.applicationForm.applyOn | getFilterPeople
          }}</template>
        </el-table-column>
        ...
        <el-table-column label="申请人公司" align="center">
          <template slot-scope="scope">
            <p>{{ scope.row.applicationForm.applyOn | getFilterCompany }}</p>
            &nbsp;
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="160" align="center">
          <template slot-scope="scope">
            <p>
              ...
              <el-button
                v-if="scope.row.applicationForm.applyStatus === 0"
                size="mini"
...
import { fetchAllList } from "@/api/login";
import { companyAll } from "@/api/company";
let than;
...
export default {
  ...
  filters: {
    //公司
    getFilterCompany:function(value) {
     
    let com=[];
     than.allCompanyList.forEach((item) => {
         if(item.id == value){
        com.push(item)
    ...
    //申请人
    getFilterPeople:function(value) {
    console.log(value)
    return   than.peo&& than.peo[value]
  ...
  methods: {
    ...
    getAllPeople() {
      ...
      this.allPeoples.forEach((item) => {
        
         this.peo[item.id]=item.username;
    ...
    handleUpdateProduct(index, row) {
      console.log(row.id);
      this.$router.push({
        path: "/oms/updateApplication",
        query: { id: row.applicationForm.id },
      });
    },
