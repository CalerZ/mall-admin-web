  ...
  <div style="margin-top: 50px; width: 100%;">
    ...
    >
      <el-card class="operate-container" shadow="never">
        ...
        <span>物品条目</span>
        <el-button class="btn-add" @click="handleDelProduct()" size="mini">
          删除
        </el-button>
          ...
          <el-table-column label="现有库存" width="120" align="center">
            <template slot-scope="scope">
              <div class="input-box">
                <el-input
                  size="small"
                  v-model="scope.row.onHandInventory"
                ></el-input>
              </div>
            </template>
          </el-table-column>
          <el-table-column label="报告数量" width="120" align="center">
            <template slot-scope="scope">
              <div class="input-box">
                <el-input
                  size="small"
                  v-model="scope.row.reportedQuantity"
                ></el-input>
              </div>
            </template>
          </el-table-column>
      ...
      >
        <product-view-list
          ref="procedureEdit"
          :view="true"
          @selectProduct="handleOk"
        ></product-view-list>
        <el-row align="center">
          <el-button type="primary" @click="handleOk" size="small" plain
            >确定</el-button
          >
      ...
      <el-form-item style="text-align: center;">
        <el-button size="medium" @click="handlePrev"
          >上一步，填写申请信息</el-button
        >
        <el-button
          size="medium"
          @click="handleAddApplication('productInfoForm')"
          >保存申请单</el-button
        >
        <el-button
          type="primary"
          size="medium"
          @click="handleSubmitApplicaton('productInfoForm')"
          >提交申请单</el-button
        >
      </el-form-item>
    </el-form>
...
        },
  ...
  methods: {
    ...
    handleOk() {
      ...
      getApplicationProductList(productIdList).then((response) => {
        //返回一个列表
    ...
    handleDelProduct() {
      ...
      this.multipleSelection.forEach((pid) => {
        let index = this.value.applicationProducts.findIndex(
          (item) => item.productId == pid.productId
        );
